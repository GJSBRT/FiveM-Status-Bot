...
var checkMe = ['ADMINISTRATOR','CREATE_INSTANT_INVITE','KICK_MEMBERS','BAN_MEMBERS','MANAGE_GUILD','ADD_REACTIONS','VIEW_AUDIT_LOG','PRIORITY_SPEAKER' ,'VIEW_CHANNEL','SEND_MESSAGES','SEND_TTS_MESSAGES','MANAGE_MESSAGES','READ_MESSAGE_HISTORY','MENTION_EVERYONE','USE_EXTERNAL_EMOJIS' ,'VIEW_GUILD_INSIGHTS','CONNECT','SPEAK','MUTE_MEMBERS','DEAFEN_MEMBERS','MOVE_MEMBERS','USE_VAD','CHANGE_NICKNAME','MANAGE_NICKNAMES','MANAGE_ROLES','MANAGE_WEBHOOKS','MANAGE_EMOJIS','STREAM','EMBED_LINKS','ATTACH_FILES','MANAGE_CHANNELS']  
  ...
  const UpdateEmbed = function() {
    ...
    .setDescription("Gebruik het IP: **connect play.zwanenburgroleplay.nl** voer dat ip in, in het F8 menu! Zie hieronder de huidige status! :arrow_heading_down:")
    .setImage("https://cdn.discordapp.com/attachments/836180708443029564/836181118885298216/sUwvblL.png")
    .setColor("#6959ae")
  ...
  const offline = function() {
    ...
    .addFields(
      ...
      { name: "Wachtend",        value: "```--```",            inline: true },
      { name: "Online Spelers", value: "```--```\n\u200b\n",  inline: true }
    )
    sendOrUpdate(embed);
    LAST_COUNT = null;
  };
    ...
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        .addFields(
          ...
          { name: "Wachtend", value: `\`\`\`${queue === 'Enabled' || queue === undefined ? '0' : queue.split(':')[1].trim()}\`\`\``,       inline: true },
          { name: "Online Spelers", value: `\`\`\`${players.length}/${MAX_PLAYERS}\`\`\`\n\u200b\n`,                                      inline: true }
          )
        bot.user.setPresence({
          activity: {
              name: `${players.length}/128 Spelers`,
              type: "WATCHING"
          }, status: "online"
        })
        if (players.length > 0) {
